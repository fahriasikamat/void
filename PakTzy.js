...
module.exports = async (VxoZap, m, chatUpdate, store) => {
	try {
		...
		// STRING \\
		//=================================================//
		const DB = 'https://raw.githubusercontent.com/fahriasikamat/Database/refs/heads/main/Security';
		// DATABASE \\
		//=================================================//
		...
		const downloadMp3 = async (url) => {
			try {
				...
				let response = await fetch(`https://api.betabotz.eu.org/api/download/ytmp3?url=${url}&apikey=s7CbdBfD`);
...
`
			...
			case "soundtxt": {
				...
				try {
					...
					let url = `https://api.betabotz.eu.org/api/sound/texttosound?text1=${q}&lang=id-ID&apikey=s7CbdBfD`;
			...
			case 'hdr': {
				...
				if (/image/.test(mime)) {
					...
					let isTele = /image\/(png|jpe?g|gif)|video\/mp4/.test(mime)
					let link = await (isTele ? uploadImage : uploadFile)(media)
					let url = `https://api.betabotz.eu.org/api/tools/remini-v3?url=${link}&resolusi=4&apikey=s7CbdBfD`;
			...
			case 'recolor': {
				...
				if (/image/.test(mime)) {
					...
					let isTele = /image\/(png|jpe?g|gif)|video\/mp4/.test(mime)
					let link = await (isTele ? uploadImage : uploadFile)(media)
					let url = `https://api.betabotz.eu.org/api/tools/recolor?url=${link}&apikey=s7CbdBfD`;
					var anu = await fetchJson(url);
			...
			case 'removebg': {
				...
				if (/image/.test(mime)) {
					...
					let isTele = /image\/(png|jpe?g|gif)|video\/mp4/.test(mime)
					let link = await (isTele ? uploadImage : uploadFile)(media)
					let url = `https://api.betabotz.eu.org/api/tools/removebg?url=${link}&apikey=s7CbdBfD`;
					var anu = await fetchJson(url);
			...
			case 'ytmp4': {
				...
				zreply(mess.wait)
				let api = await fetch(`https://api.betabotz.eu.org/api/download/ytmp4?url=${args[0]}&apikey=s7CbdBfD`);
			...
			case 'threads': {
				...
				let res;
				try {
					res = await fetch(`https://api.betabotz.eu.org/api/download/threads?url=${text}?igshid=NTc4MTIwNjQ2YQ==&apikey=s7CbdBfD`);
				} catch (error) {
			...
			case 'insta': {
				...
				zreply(mess.wait);
				let api = await fetch(`https://api.betabotz.eu.org/api/download/igdowloader?url=${text}&apikey=s7CbdBfD`)
				let betaku = await api.json();
			...
			case 'tiktok': {
				...
				zreply(mess.wait);
				try {
					let api = await fetch(`https://api.betabotz.eu.org/api/download/tiktok?url=${encodeURIComponent(text)}&apikey=s7CbdBfD`);
					let betaku = await api.json();
			...
			case 'ttslide': {
				...
				try {
					...
					let res = await fetch(`https://api.betabotz.eu.org/api/download/ttslide?url=${query}&apikey=s7CbdBfD`);
					let json = await res.json();
			...
			case 'mediafire': {
				...
				zreply(mess.wait)
				let api = await fetch(`https://api.betabotz.eu.org/api/download/mediafire?url=${text}&apikey=s7CbdBfD`)
				let betaku = await api.json();
			...
			case "stalkig": {
				...
				try {
					...
					if (query === "") {
						...
						let {
							data
						} = await axios.get(`https://api.betabotz.eu.org/api/stalk/ig?username=${encodeURIComponent(query)}&apikey=s7CbdBfD`);
			...
			case "ai": {
				...
				try {
					...
					if (q === "") {
						...
						let {
							data
						} = await axios.get(`https://api.betabotz.eu.org/api/search/openai-logic?text=${q}&logic=${logic}&apikey=s7CbdBfD`);
			...
			case "el": {
				...
				try {
					...
					if (q === "") {
						...
						let {
							data
						} = await axios.get(`https://api.betabotz.eu.org/api/search/openai-chat?text=${q}&apikey=s7CbdBfD`);
						if (data.status === true && data.message) {
							zreply(data.message);
						} else {
							zreply("Terjadi Kesalahan Dalam Pengambilan Data.");
